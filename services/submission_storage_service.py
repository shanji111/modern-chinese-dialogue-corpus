...
def save_submission_upload(file_storage):
    return get_storage_backend().save_submission_file(file_storage)


def build_submission_download_response(
    stored_filename,
    download_name=None,
    object_key=None,
    file_url=None,
...
def delete_submission_upload(stored_filename, object_key=None, storage_backend=None):
    backend = S3StorageBackend() if storage_backend == "s3" else get_storage_backend()
    if isinstance(backend, S3StorageBackend):
